...
def valida_cnpj(cnpj):
    ...
    # verifica se o CNPJ tem 14 dígitos.Verifica o conjunto (set) dos numeros, ou seja, se apresenta apenas um conjunto, todos os digitos sao iguais
...
arquivo_operadoras = pd.read_csv(os.path.join(script_dir, 'Relatorio_cadop.csv'), sep=',', encoding='latin1', dtype={'CNPJ': str})
arquivo_operadoras = arquivo_operadoras.set_index('REGISTRO_OPERADORA').to_dict('index')
...
def processamento_dados(lista_arquivos_trimestrais): 
    lista_resultados = []
    ...
    for arquivo in lista_arquivos_trimestrais: 
        ...
        for bloco in pd.read_csv(arquivo, chunksize=100000, sep=';', encoding='UTF-8'):
            ...
            bloco['RAZAO_SOCIAL'] = bloco['RAZAO_SOCIAL'].fillna('RAZAO NAO IDENTIFICADA')
            ...
            if bloco['VL_SALDO_FINAL'].dtype == object:
                bloco['VL_SALDO_FINAL'] = bloco['VL_SALDO_FINAL'].str.replace(',', '.').apply(Decimal) # Uso do tipo de dado "decimal" para manter a precisão
            
            # Agregação local (reduz o tamanho da lista_resultados)
            lista_resultados.append(bloco)

    # Consolidação Global dos dados
    df_consolidado = pd.concat(lista_resultados, ignore_index=True)
    resultado_final = df_consolidado.groupby(['CNPJ', 'RAZAO_SOCIAL', 'ANO', 'TRIMESTRE'])['VL_SALDO_FINAL'].sum().reset_index()
...
# Chamada da função e salvamento
arquivos = [
    os.path.join(script_dir, '1T2025.csv'),
    os.path.join(script_dir, '2T2025.csv'),
    os.path.join(script_dir, '3T2025.csv')
]
df_final = processamento_dados(arquivos)

# Salvando com o encoding solicitado para evitar erros de acentuação
csv_name = os.path.join(script_dir, 'consolidado_despesas.csv')
df_final.to_csv(csv_name, index=False, sep=';', encoding='utf-8-sig')

# Compactação ZIP solicitada 
with zipfile.ZipFile(os.path.join(script_dir, 'consolidado despesas.zip'), 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write(csv_name)
